...
def prepare_digest_delivery(*, raw_text: str, output_format: str, chunk_size: int = DEFAULT_CHUNK_SIZE) -> PreparedTelegramText:
    ...
    if normalized_format == PLAIN_TEXT_PARSE_MODE:
        ...
        return PreparedTelegramText(text=text, chunks=_split_plain_text(text=text, chunk_size=chunk_size))
    ...
    text = "\n\n".join(blocks).strip()
    return PreparedTelegramText(text=text, chunks=_chunk_blocks(blocks=blocks, chunk_size=chunk_size))
