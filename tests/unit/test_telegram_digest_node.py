...
def test_digest_node_normalizes_legacy_now_placeholder_in_title_template() -> None:
    node_path = ROOT / "n8n/custom-nodes/telegram-digest/TelegramDigest.node.js"
    script = f"""
const Module = require('module');
const originalLoad = Module._load;
...
async function main() {{
  ...
  const context = {{
    ...
    getNodeParameter(name) {{
      ...
      if (name === 'outputFormat') return 'markdown_v2';
      if (name === 'includeTitle') return true;
      if (name === 'titleTemplate') return '📰 ДАЙДЖЕСТ ЗА {{$now.setLocale("uk").toFormat("d MMMM")}}';
      throw new Error(`unexpected node parameter ${{name}}`);
    }},
  }};
  await node.execute.call(context);
...
"""
    ...
    assert result.returncode == 0, result.stderr or result.stdout
    assert '{{$now.setLocale' not in result.stdout
    assert '"title_text":"📰 ДАЙДЖЕСТ ЗА ' in result.stdout
